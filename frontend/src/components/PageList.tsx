...
export function PageList({
  ...
  return (
    <div className="flex-1 overflow-y-auto p-2">
      ...
      <ul className="list-none flex flex-col gap-[0.375rem]">
        {pages.map((page, index) => (
          <li
            key={index}
            className={`flex flex-col items-center gap-1 p-[0.5rem_0.375rem] bg-bg-tertiary rounded-[0.5rem] cursor-pointer transition-[transform_opacity_background-color_border-color_box-shadow] duration-200 ease-[cubic-bezier(0.2,0,0,1)] border-2 border-transparent relative group hover:bg-bg-elevated ${selectedIndex === index ? 'border-accent bg-[rgba(136,192,208,0.15)]' : ''}`}
          ...
          >
            ...
            >
              <span className={`w-9 h-9 flex items-center justify-center bg-bg-secondary rounded-[0.5rem] text-base font-semibold text-text-secondary transition-all duration-200 group-hover/icon:blur-[1px] ${selectedIndex === index ? 'bg-accent text-bg-primary' : ''}`}>
                {getPageInitial(page.name)}
              </span>
              {selectedIndex === index && (
                <span className="absolute inset-0 flex items-center justify-center bg-[rgba(136,192,208,0.95)] rounded-[0.5rem] opacity-0 transition-opacity duration-200 text-bg-primary backdrop-blur-[2px] shadow-[0_0_12px_rgba(136,192,208,0.5)] group-hover/icon:opacity-100">
                  <Pencil size={12} />
                </span>
              )}
            </div>
            ...
            {editingIndex === index ? (
              <input
                type="text"
                className="w-full p-[0.125rem_0.25rem] bg-bg-primary border border-accent rounded-[0.25rem] text-text-primary text-[0.65rem] text-center"
                value={editingName}
